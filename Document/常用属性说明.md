...
## DGCJXSegmentedViewDataSource协议类常用属性

### JXSegmentedBaseDataSource常用属性

属性     | 说明           |
--------------|---------------|
dataSource | 最终传递给DGCJXSegmentedView的数据源数组 |
itemContentWidth    | cell的内容宽度，为DGCJXSegmentedViewAutomaticDimension时就以内容计算的宽度为准，否则以itemContentWidth的具体值为准。 |
itemWidthIncrement  |   真实的item宽度 = itemContentWidth + itemWidthIncrement。  |
itemSpacing | item之前的间距 |
isItemSpacingAverageEnabled | 当collectionView.contentSize.width小于DGCJXSegmentedView的宽度时，是否将itemSpacing均分。 |
isItemTransitionEnabled | item左右滚动过渡时，是否允许渐变。比如JXSegmentedTitleDataSource的titleZoom、titleNormalColor、titleStrokeWidth等渐变。   |
isSelectedAnimable  | 选中的时候，是否需要动画过渡。自定义的cell需要自己处理动画过渡逻辑，动画处理逻辑参考`DGCJXSegmentedTitleCell` |
...
dotSize | 红点的size |
dotCornerRadius | 红点的圆角值，DGCJXSegmentedViewAutomaticDimension等于dotSize.height/2 |
dotColor | 红点的颜色 |
...
属性     | 说明           |
--------------|---------------|
indicatorWidth | 默认DGCJXSegmentedViewAutomaticDimension（与cell的宽度相等）。内部通过getIndicatorWidth方法获取实际的值 |
indicatorHeight | 默认DGCJXSegmentedViewAutomaticDimension（与cell的高度相等）。内部通过getIndicatorHeight方法获取实际的值 | 
indicatorCornerRadius | 默认DGCJXSegmentedViewAutomaticDimension （等于indicatorHeight/2）。内部通过getIndicatorCornerRadius方法获取实际的值 |
...
使用继承于`DGCJXSegmentedIndicatorBaseView`的属性即可
