//
//  DGCListCacheViewController.swift
...
class DGCListCacheViewController: UIViewController {
    ...
    private var dgc_listCacheDict: [String: DGCLoadDataListViewController] = [:]
...
extension DGCListCacheViewController: DGCJXSegmentedListContainerViewDataSource {
    ...
    func listContainerView(_ listContainerView: DGCJXSegmentedListContainerView, initListAt index: Int) -> DGCJXSegmentedListContainerViewListDelegate {
        let dgc_title = segmentedDataSource.titles[index]
        if let dgc_vc = dgc_listCacheDict[dgc_title] {
            return dgc_vc
        } else {
            let dgc_vc = DGCLoadDataListViewController()
            dgc_vc.typeString = dgc_title
            dgc_listCacheDict[dgc_title] = dgc_vc
            return dgc_vc
        }
    }
}
