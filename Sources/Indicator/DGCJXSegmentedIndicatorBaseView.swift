//
//  DGCJXSegmentedIndicatorBaseView.swift
//  DGCJXSegmentedView
//
...
public enum DGCJXSegmentedIndicatorPosition {
    case top
    case bottom
    case center
}

open class DGCJXSegmentedIndicatorBaseView: UIView, DGCJXSegmentedIndicatorProtocol {
    /// 默认DGCJXSegmentedViewAutomaticDimension（与cell的宽度相等）。内部通过getIndicatorWidth方法获取实际的值
    open var indicatorWidth: CGFloat = DGCJXSegmentedViewAutomaticDimension
    ...
    /// 默认DGCJXSegmentedViewAutomaticDimension（与cell的高度相等）。内部通过getIndicatorHeight方法获取实际的值
    open var indicatorHeight: CGFloat = DGCJXSegmentedViewAutomaticDimension
    /// 默认DGCJXSegmentedViewAutomaticDimension （等于indicatorHeight/2）。内部通过getIndicatorCornerRadius方法获取实际的值
    open var indicatorCornerRadius: CGFloat = DGCJXSegmentedViewAutomaticDimension
    /// 指示器的颜色
    open var indicatorColor: UIColor = .red
    /// 指示器的位置，top、bottom、center
    open var indicatorPosition: DGCJXSegmentedIndicatorPosition = .bottom
    ...
    ///  指示器的宽度是否跟随item的内容变化（而不是跟着cell的宽度变化）。indicatorWidth=DGCJXSegmentedViewAutomaticDimension才能生效
    ...
    public func getIndicatorCornerRadius(itemFrame: CGRect) -> CGFloat {
        if indicatorCornerRadius == DGCJXSegmentedViewAutomaticDimension {
    ...
    public func getIndicatorHeight(itemFrame: CGRect) -> CGFloat {
        if indicatorHeight == DGCJXSegmentedViewAutomaticDimension {
            return itemFrame.size.height
        }
        return indicatorHeight
    }

    public func canHandleTransition(model: DGCJXSegmentedIndicatorTransitionParams) -> Bool {
    ...
    public func canSelectedWithAnimation(model: DGCJXSegmentedIndicatorSelectedParams) -> Bool {
    ...
    //MARK: - DGCJXSegmentedIndicatorProtocol
    open func refreshIndicatorState(model: DGCJXSegmentedIndicatorSelectedParams) {
    }

    open func contentScrollViewDidScroll(model: DGCJXSegmentedIndicatorTransitionParams) {
    }

    open func selectItem(model: DGCJXSegmentedIndicatorSelectedParams) {
    }
}
