...
open class DGCJXSegmentedBaseCell: UICollectionViewCell, DGCJXSegmentedViewRTLCompatible {
    open var itemModel: DGCJXSegmentedBaseItemModel?
    open var animator: DGCJXSegmentedAnimator?
    private var dgc_selectedAnimationClosureArray = [JXSegmentedCellSelectedAnimationClosure]()
...
//        }
    ...
    open func canStartSelectedAnimation(itemModel: DGCJXSegmentedBaseItemModel, selectedType: DGCJXSegmentedViewItemSelectedType) -> Bool {
        var dgc_isSelectedAnimatable = false
        if itemModel.isSelectedAnimable {
            if selectedType == .scroll {
                //滚动选中且没有开启左右过渡，允许动画
                if !itemModel.isItemTransitionEnabled {
                    dgc_isSelectedAnimatable = true
                }
        ...
        return dgc_isSelectedAnimatable
    }

    open func appendSelectedAnimationClosure(closure: @escaping JXSegmentedCellSelectedAnimationClosure) {
        dgc_selectedAnimationClosureArray.append(closure)
    }

    open func startSelectedAnimationIfNeeded(itemModel: DGCJXSegmentedBaseItemModel, selectedType: DGCJXSegmentedViewItemSelectedType) {
        if itemModel.isSelectedAnimable && canStartSelectedAnimation(itemModel: itemModel, selectedType: selectedType) {
            ...
            animator?.progressClosure = {[weak self] (percent) in
                ...
                for closure in self!.dgc_selectedAnimationClosureArray {
                    closure(percent)
                }
            }
            animator?.completedClosure = {[weak self] in
                itemModel.isTransitionAnimating = false
                self?.dgc_selectedAnimationClosureArray.removeAll()
            }
            animator?.start()
        }
    }

    open func reloadData(itemModel: DGCJXSegmentedBaseItemModel, selectedType: DGCJXSegmentedViewItemSelectedType) {
        if segmentedViewShouldRTLLayout() {
//            segmentedView(horizontalFlipForView: self)
            segmentedView(horizontalFlipForView: contentView)
        }else{
        ...
        if itemModel.isSelectedAnimable {
            dgc_selectedAnimationClosureArray.removeAll()
            if canStartSelectedAnimation(itemModel: itemModel, selectedType: selectedType) {
                animator = DGCJXSegmentedAnimator()
